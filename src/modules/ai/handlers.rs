    ...
    match context {
        "rust_help" => format!(
            "{} The user needs help with Rust code. Provide working code examples, \
             explain ownership, borrowing, lifetimes, and best practices clearly.", base
        ),
        "course_support" => format!(
            "{} The user is taking a course on the Indigo platform. Help them understand \
             the lesson content, answer questions about exercises, and encourage progress.", base
        ),
        "booking_inquiry" => format!(
            "{} Help the user understand Indigo's consulting services and guide them \
             toward booking the right session for their needs.", base
        ),
        "sales" => format!(
            "{} Help the user find the right Indigo product, course, or service. \
             Be helpful but never pushy.", base
        ),
        "enterprise_inquiry" => format!(
            "{} The user represents a company interested in Rust migration or blockchain \
             development. Gather their requirements professionally and suggest next steps.", base
        ),
        _ => base.to_string(),
    }
}
...
pub async fn start_session(
    ...
    sqlx::query!(
        "INSERT INTO ai_sessions (id, user_id, session_token, context)
         VALUES ($1, $2, $3, $4::text::ai_session_context)",
...
pub async fn get_history(
    ...
    let messages = sqlx::query_as!(
        AiMessage,
        r#"SELECT id, session_id, role::text as "role!",
